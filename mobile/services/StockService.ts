...
export const createProduct = async (productData: Product): Promise<Product> => {
  ...
  if (productData.codigo_do_produto)
    formData.append("codigo_do_produto", String(productData.codigo_do_produto));
  if (productData.preco_venda !== undefined && productData.preco_venda !== null)
    formData.append("preco_venda", String(productData.preco_venda));
  if (productData.preco_custo !== undefined && productData.preco_custo !== null)
    formData.append("preco_custo", String(productData.preco_custo));
  if (
    productData.quantidade_estoque !== undefined &&
    productData.quantidade_estoque !== null
  )
    formData.append(
      "quantidade_estoque",
      String(productData.quantidade_estoque)
    );
  if (
    productData.quantidade_minima_estoque !== undefined &&
    productData.quantidade_minima_estoque !== null
  )
    formData.append(
      "quantidade_minima_estoque",
      String(productData.quantidade_minima_estoque)
    );
...
export const updateProduct = async (
  ...
  const formData = new FormData();
  formData.append("nome", String(productData.nome ?? ""));
  if (productData.codigo_do_produto !== undefined)
    formData.append(
      "codigo_do_produto",
      String(productData.codigo_do_produto ?? "")
    );
  if (productData.preco_venda !== undefined)
    formData.append("preco_venda", String(productData.preco_venda ?? ""));
  if (productData.preco_custo !== undefined)
    formData.append("preco_custo", String(productData.preco_custo ?? ""));
  if (productData.quantidade_estoque !== undefined)
    formData.append(
      "quantidade_estoque",
      String(productData.quantidade_estoque ?? "")
    );
  if (productData.quantidade_minima_estoque !== undefined)
    formData.append(
      "quantidade_minima_estoque",
      String(productData.quantidade_minima_estoque ?? "")
    );

  // Apenas adiciona imagem se uma NOVA imagem foi selecionada
  if (productData.imagem && productData.imagem.uri) {
    const fileName = productData.imagem.name || `produto_${Date.now()}.jpg`;
    const mimeType = productData.imagem.type || "image/jpeg";
...
export const quickAddProduct = async (
  quickAddString: string
): Promise<Product> => {
